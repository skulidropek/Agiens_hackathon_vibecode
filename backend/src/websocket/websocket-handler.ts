...
function broadcastTerminalListUpdate(terminalService: TerminalService, projectId?: string): void {
...
export const setupWebSocketRoutes = (wss: WebSocketServer, projectService: ProjectService, fileWatcherService: FileWatcherService, terminalService: TerminalService): void => {
  const terminalHandler = new TerminalHandler(terminalService);
  
  // Устанавливаем callback для обновления списка терминалов
  terminalService.onTerminalListUpdate = () => broadcastTerminalListUpdate(terminalService);
  ...
  async function handleTerminalListSubscription(connection: WebSocketConnection, message: { type: string; payload: { projectId?: string } }): Promise<void> {
    try {
      ...
      logger.info('Processing terminal list subscription', {
        connectionId: connection.id,
        projectId
      });

      // Помечаем соединение как подписчика на список терминалов
      ...
      sendErrorMessage(connection.ws, 'Failed to subscribe to terminal list', 'TERMINAL_ERROR');
    }
  }
  ...
  async function handleTerminalListUnsubscription(connection: WebSocketConnection, _message: { type: string; payload: Record<string, never> }): Promise<void> {
    try {
      logger.info('Processing terminal list unsubscription', {
        connectionId: connection.id
      });

      // Убираем пометку подписчика
      ...
      sendErrorMessage(connection.ws, 'Failed to unsubscribe from terminal list', 'TERMINAL_ERROR');
    }
  }
